...
class Dense(object):
    ...
    def __encode(self, cells, seq, ft, config, training=True):
        ...
        model = BertModel(
            ...
            scope='bert')
        embedding1 = model.get_sequence_output()

        noise = [tf.shape(embedding1)[0], 1, tf.shape(embedding1)[2]]
        embedding = tf.layers.dropout(embedding1, 0.5, noise, training=training)

        output2 = cells(embedding)
        output2 = tf.concat([output2, embedding1, ft], axis=-1)

        h_v = tf.layers.dense(output2, config.pos_size, name='dense', reuse=tf.AUTO_REUSE)[:, 1:-1, :]
